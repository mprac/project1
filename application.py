...
def index():
    if 'user' in session:
        ...
        if request.method == "POST":
            ...
            if search:
                results = db.execute("SELECT * FROM books WHERE title LIKE :title or isbn LIKE :isbn or author LIKE :author LIMIT 20", {
                                     "title": search, "isbn": search, "author": search})
    ...
    if request.method == "POST":
        ...
        try:
            db.execute("INSERT INTO users (email, username, password) VALUES (:email, :username, :password)", {
                       "email": email, "username": username, "password": encryptPass})
        except:
            return render_template("auth/register.html", message="email or username already exist")
        db.commit()
        return render_template('auth/login.html', success="You are now registered, please login here!")
...
def book(id):
    if 'user' in session:
        ...
        # return number to 2 deciimal places using Trunc()
        rating_average = db.execute(
            'SELECT TRUNC(AVG(rating), 2) FROM reviews WHERE book_id=:id', {'id': id})
        # get goodreads ratings
        isbn = db.execute('SELECT isbn FROM books WHERE id=:id', {
                          'id': id}).fetchone()
        res = requests.get("https://www.goodreads.com/book/review_counts.json",
                           params={"key": "eNLoXytUL2ZxAVY6gxVwEw", "isbns": isbn})
        gr_data = res.json()
...
def review():
    if 'user' in session:
        ...
        if request.method == "POST":
            ...
            # return all reviews for specified user id to run IF statements and return error messages or execute the review to database.
            ...
            if reviewed:
                ...
                return redirect(url_for('book', id=book_id))
            # strip removes white space if user clicks on spacebar and trys to submit empty string
            elif not review.strip():
                flash('Cannot submit empty spaces')
                return redirect(url_for('book', id=book_id))
            else:
...
def goodreadsapi(isbn):
    if 'user' in session:
        ...
        if request.method == "GET":
            ...
            # return number to 2 decimal places using Trunc() - match book ids from reviews table and books table. fetchone isbn from books table to match
            average_score = db.execute(
                'SELECT TRUNC(AVG(rating), 2) FROM reviews JOIN books ON reviews.book_id = books.id AND isbn=:isbn', {'isbn': isbn}).fetchone()
            # used COUNT to calculate total reviews from the reviews table by matching reviews table book id with books table id and isbn
            review_count = db.execute(
                'SELECT COUNT(rating) FROM reviews JOIN books ON reviews.book_id = books.id AND isbn=:isbn', {'isbn': isbn})
            if book is None:
                # return 404 error page
                abort(404)
            for row in average_score:
                if row:
                    # store the trunc number as a float
                    score = float(row)
                else:
                    score = 0
            for row in review_count:
                if row:
                    ...
                    count = 0
            # return the data to website.com/api/bookISBN
            return jsonify({
                "title": book.title,
                "author": book.author,
                "year": book.year,
