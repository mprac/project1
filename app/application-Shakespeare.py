...
def login():
    ...
    #check if request is post. if not the it us get.  
    if request.method == "POST":
        ...
        # email=request.form.get("email")
        # password=request.form.get("password")
        # user = db.execute("SELECT password FROM users WHERE email= :email", {"email": email}).fetchone()

       

        # if db.execute("SELECT email, password FROM users WHERE email=:email",{'email':email}).fetchone() or check_password_hash(user.password, password) == False:
